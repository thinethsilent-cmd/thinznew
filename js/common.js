...
export function loadBotPage() {
  ...
  if (tradesList) tradesList.innerHTML = `<tr><td colspan="5" class="text-center py-4 text-gray">Loading execution logs...</td></tr>`;
  activeUnsubscribes.trades = subscribeToTrades(state.user.uid, trades => {
    if (!tradesList) return;
    tradesList.innerHTML = '';
    if (trades.length === 0) {
      tradesList.innerHTML = `<tr><td colspan="5" class="text-center py-4 text-gray">No trades executed yet.</td></tr>`;
